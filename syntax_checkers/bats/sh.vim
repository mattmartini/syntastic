...
if exists('g:loaded_syntastic_bash_sh_checker')
    finish
endif
let g:loaded_syntastic_bash_sh_checker = 1

let s:save_cpo = &cpo
set cpo&vim

function! SyntaxCheckers_bash_sh_IsAvailable() dict " {{{1
    let buf = bufnr('')
    call self.log('shell =', s:GetShell(buf))
    return s:IsShellValid(buf)
endfunction " }}}1

function! SyntaxCheckers_bash_sh_GetLocList() dict " {{{1
    let buf = bufnr('')
